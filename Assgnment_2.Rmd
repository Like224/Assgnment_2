...
Overall, we have highly heterogeneous effect size data because At an accuracy of five decimal places, sampling changes barely affect the change in total effect. The $I_{Total}^2$ is `r i2[1, "I2"]` From the multilevel meta-analytic model we found that only `r i2[2, "I2"]` of the total variation in effect size estimates was due to between-study variation.
...
metafor::funnel(x = rom_data$lnRR, vi= rom_data$V_lnRR, yaxis = "seinv", xlab = "Log Response Ratio (lnRR)",level = c(0.1,0.05,0.01) ,shade = c("white", "gray55", "gray 75"),
    las = 1, ylim = c(0.1, 40), xlim = c(-4, 4),  legend = TRUE)
```
...
```{r po, fig.cap= "Plot of lnRR against sampling variance for lnRR. A linear model was fit to the data.(X-axis is limited)"}
...
```{r TL2, fig.cap= "Plot of lnRR as a function of Print year and filter out the too-small lnRR,. Points are scaled in relation to their precision (1/sqrt(V_lnRR)). Small points indicate effects with low precision or high sampling varaince"}
